...
async def analyze_media(request: Request, file: UploadFile = File(...)):
    ...
    try:
        ...
        if file.content_type.startswith("image/"):
            ...
            prompt_text = "Тебя зовут Ингриа или Ингрия. Преобразуй аудио в текст. Отправь распознанный текст! потом напиши [моё мнение] и ответь как студентка гений Если в тексте есть вопрос к тебе (то есть обращение к Ингриа или Ингрия) то ответь на этот вопрос! Если ты поняла в тексте что есть обращение к тебе, то отвечай эмоциаонально и со словами [Привет Я Ингрия] если нет обращуения то отвечай проще! если в аудио есть помехи типа ПППППП или такие, то игнорируй их!"
            contents = [
                prompt_text,
                {"mime_type": file.content_type, "data": file_data}
            ]
