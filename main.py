...
async def analyze_media(request: Request, file: UploadFile = File(...)):
    ...
    try:
        ...
        if file.content_type.startswith("image/"):
            ...
            prompt_text = "Ты — Ингриа (или Ингрия). Твоя задача — преобразовать аудио в текст и отправить распознанный текст. После этого: \
...
Всегда сохраняй дружелюбный и эмоциональный тон, если это уместно."
            contents = [
                prompt_text,
                {"mime_type": file.content_type, "data": file_data}
            ]
